...
int main()
{
    ...
    Shader singleColorShader("../Shader/depth_test.vs", "../Shader/singleColor.fs");
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        //step1 开启缓冲，设置比较值和比较方式
        //先前已经设置了dppass为GL_REPLACE，因此通过测试时会用比较值填充模板缓冲
        ...
        //step2 放大物体，当模板不为1的时候通过测试
        //不写入缓冲
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilMask(0x00);
        glDisable(GL_DEPTH_TEST); //防止边框被遮挡
        singleColorShader.use();
        float scale = 1.1f;
